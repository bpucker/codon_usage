...
__usage__ = """
	python get_translational_bottle_necks.py
...
def construct_figure( fig_file, ID, values, window_size ):
	...
	for i in range( len( values )-window_size ):
		values_to_plot.append( sum( values[i:i+window_size] ) / float( window_size ) )
	ax.plot( values_to_plot )
...
def main( arguments ):
	...
	if '--win' in arguments:
		window_size = int( arguments[ arguments.index( '--win' )+1 ] )
	else:
		window_size = 4
	...
	for ID in codon_usage_per_seq.keys():
		...
		construct_figure( fig_file, ID, codon_usage_per_seq[ ID ], window_size )
